% Read the STL file
vertices = stlread('super_simple.stl');

separated = splitFV(vertices.ConnectivityList, vertices.Points);

% Plot one
idx = 6;
single_component = triangulation(separated(idx).faces, separated(idx).vertices);
% model= createpde;
% geometryFromMesh(model, ...
%     single_component.Points', ...
%     single_component.ConnectivityList');
% generateMesh(model, 'Hmax', 1.0);
% pdemesh(model);
trisurf(single_component);
axis image;

function subdivide(points, triangles, max_edge_length)
    edges = zeros([1, 3 * length(triangles)]);
    edge_test = zeros([3 2]);
    edge_counts = 0;
    for idx = 1:length(triangles)
        edge_test(1, :) = triangles(idx, [1 2]);
        edge_test(2, :) = triangles(idx, [2 3]);
        edge_test(3, :) = triangles(idx, [3 1]);
        for jdx = 1:edge_counts
            for kdx = 1:3
                if all(edge_test(kdx, :) == triangles(idx, :))
