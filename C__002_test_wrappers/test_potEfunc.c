...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
      ...
      double step_times[time_steps];
      for(int i = 0; i < time_steps; i++)
            step_times[i] = step_times_in[i] * 1.0e-6;

      field_at(maxt, voltages, step_times, \
           n_electrodes, time_steps,\
           xx, yy, zz, din, potential_maps, dimensions,\
           ex, ey, ez);
      plhs[7] = mxCreateDoubleScalar(its);
      plhs[ 8] = mxCreateDoubleScalar(ex);
      plhs[ 9] = mxCreateDoubleScalar(ey);
      plhs[10] = mxCreateDoubleScalar(ez);
}