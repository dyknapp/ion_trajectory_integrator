...
initial_angle =   0. * (pi/180.);    % radians
collimation_f = 100000.;             % mm      : Distance at which the cloud should be focused
initial_location = [5.0 10.0];       % mm      : Center of cloud
initial_spread = 1.0;                % mm      : Standard deviation of Gaussian position initialization
cloud_temperature =   1.0;           % Kelvin  : Initial temperature of cloud

% Simulation parameters
end_time   =  15.0;                  % us : Time cutoff for simulation
...
simion_path = "SIM001_data/010_einzel_lens";

% Names of individual files containing electrodes' potentials
electrode_names = ["einzel.pa1.patxt", ...
                   "einzel.pa2.patxt", ...
                   "einzel.pa3.patxt", ...
                  ];
...
% imagesc(d*(0:dimensions(1)-1), d*(1:dimensions(2)-1), squeeze(potential_maps(2, :, :)))
...
if cylindrical
    potential_maps = permute(potential_maps, [1 3 2]);
...
figure
imagesc(d*(0:dimensions(1)-1), d*(1:dimensions(2)-1), is_electrode) %squeeze(potential_maps(2,:,:)))
xlabel("x (mm)"); ylabel("y (mm)"); set(gca,'YDir','normal'); colormap("gray");
axis image
hold on
for idx = 1:particles
